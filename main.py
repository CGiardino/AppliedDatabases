from services.attendee_service import update_attendee, add_attendee_connection, add_attendee, \
    show_attendees_by_company_id, fetch_connected_attendees, delete_attendee, show_connected_attendees_same_session
...
def _display_menu() -> None:
    ...
    print('9 - Show Connected Attendees in Same Session')
...
def _run_menu() -> None:
    ...
    while True:
        ...
        if choice == '9':
            try:
                show_connected_attendees_same_session()
            except Exception as exc:
                print(f'{ERROR_PREFIX} Could not show connected attendees in same session: {exc}\n')
            continue
